...
function TagsInputLabel({
  ...
  return (
    <TagsInputPrimitive.Label
      ...
      className={cn(
        "font-medium text-sm leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70 text-white font-sora",
        className,
      )}
      {...props}
    />
...
function TagsInputList({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn(
        "flex min-h-10 w-full flex-wrap items-center gap-1.5 rounded-md border border-[#303030] text-sm disabled:cursor-not-allowed disabled:opacity-50 p-5",
        className,
      )}
      {...props}
    />
...
function TagsInputInput({
  ...
  return (
    <TagsInputPrimitive.Input
      ...
      className={cn(
        "flex-1 bg-transparent outline-hidden placeholder:text-muted-foreground disabled:cursor-not-allowed disabled:opacity-50 text-white",
        className,
      )}
      {...props}
    />
...
function TagsInputItem({
  ...
  return (
    <TagsInputPrimitive.Item
      ...
      className={cn(
        "inline-flex max-w-[calc(100%-8px)] items-center gap-1.5 rounded border border-[#303030] bg-transparent px-2.5 py-1 text-sm focus:outline-hidden data-disabled:cursor-not-allowed data-editable:select-none data-editing:bg-transparent data-disabled:opacity-50 data-editing:ring-1 data-editing:ring-ring [&:not([data-editing])]:pr-1.5 [&[data-highlighted]:not([data-editing])]:bg-accent [&[data-highlighted]:not([data-editing])]:text-accent-foreground text-white",
        className,
      )}
      {...props}
    >
